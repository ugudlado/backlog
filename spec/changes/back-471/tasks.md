...
- [x] T-7 Write tests: `backlog workspace doctor` CLI integration — healthy registry exits 0; broken registry exits 1; `--fix` prompts via clack and prunes on confirmation; `--yes` skips prompt (RED) (depends: T-6)
...
- [x] T-8 Implement: `program.command("workspace")` parent + `doctor [--fix] [--yes]` subcommand wiring (GREEN) (depends: T-7)
...
- [x] T-10 Write tests: MCP `workspace_list` and `workspace_switch` handlers — happy paths + unknown id error (RED) (depends: T-2)
...
- [x] T-11 Implement: `src/mcp/tools/workspaces/{index.ts,handlers.ts,schemas.ts}`; register in `src/mcp/server.ts` (GREEN) (depends: T-10)
...
- [x] T-12 Refactor: ensure handler logic shares the registry lock path through `setCurrentWorkspaceId` (no new lock paths introduced) (depends: T-11)
...
- [x] T-13 Write tests: `agent-guidelines.md` snapshot includes the "Workspace registry" section; `addAgentInstructions` outputs reference `workspace_list` / `workspace_switch` and `backlog workspace doctor` (RED) (depends: T-11)
  - **Why**: AC-7 (FR-7).
  - **Verify**: Snapshot / content tests FAIL — section not yet added.

- [x] T-14 Implement: add "Workspace registry" section to `src/guidelines/agent-guidelines.md`; update any snapshot fixtures (GREEN) (depends: T-13)
...
- [x] T-15 Final review checkpoint (phase gate) (depends: T-14, T-12, T-9, T-4)
